  ...
  <DialogPortal>
    <DialogOverlay>
      <DialogPrimitive.Content
        ref={ref}
        className={ny(
          "z-50 gap-4 w-full relative rounded-b-lg border border-border bg-background shadow-xl animate-in data-[state=open]:fade-in-90 data-[state=open]:slide-in-from-bottom-10 sm:max-w-lg sm:rounded-lg sm:zoom-in-90 data-[state=open]:sm:slide-in-from-bottom-0 data-[state=open]:my-5",
          className
        )}
        {...props}
      >
        {children}
        {!withoutClose && (
          <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent text-foreground">
            <X size={25} />
